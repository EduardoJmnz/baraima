...
function updateProcessBlock() {
  ...
  if (historyBottle) {
    ...
    if (processVisible) {
      ...
      const historyWidth = isMobileMotion ? Math.min(vw * 0.46, 228) : Math.min(Math.max(vw * 0.276, 290), 456);
      const processWidth = isMobileMotion ? Math.min(vw * 0.60, 300) : Math.min(Math.max(vw * 0.36, 420), 610);
      const historyRight = isMobileMotion ? 0 : (vw < 980 ? (vw < 620 ? vw * 0.04 : vw * 0.08) : Math.min(Math.max(vw * 0.18, 150), 290));
      // Mobile uses translateX(-50%) in CSS, so the shared position is the can center.
      // Desktop keeps the legacy left-edge positioning.
      const startLeft = isMobileMotion ? vw * 0.50 : vw - historyRight - historyWidth;
      const endLeft = isMobileMotion ? vw * 0.20 : Math.max(-80, vw * 0.07);
      ...
      const startTop = isMobileMotion ? vh * 0.72 : desktopStartTop;
      const endTop = isMobileMotion ? vh * 0.60 : vh * 0.49 + 75;
      const startRotate = 0;
      const endRotate = isMobileMotion ? -4 : -11;
